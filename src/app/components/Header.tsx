  ...
  return (
    <header className="fixed z-50 top-0 left-0 right-0">
      <nav className="flex   max-w-container justify-between font-subtitle  text-subtitle mx-auto ">
        <div className="sm:flex-1 py-5 px-5 sm:px-0 z-20  items-center sm:flex ">
          <ThemeSwitcher />
        </div>
        {/*menu mobile */}
        <div ref={menu} className="min-w-[100vw] transition-all ease-in-out duration-500 translate-x-[100vw] max-w-container flex absolute top-0 left-0 right-0    mx-auto  h-[100vh] ">
          ...
          <div className="w-[60%] max-w-[400px]  text-subtitle py-5 sombraMenu bg-background">
            <ul className="flex flex-col gap-5  text-center">
              <div className="flex justify-center " onClick={handleMenu}><Image src={`/Header/close.svg`} width={18} height={18} className="hover:scale-105 cursor-pointer transition-all ease-in-out duration-300"  alt='close' /></div>
              {liNav.map((data, id) => (
                <li key={id}>
                  <Link href={`/${data.slug}`}>
        ...
        <div
          onClick={handleMenu}
          className="flex cursor-pointer group  px-5 sm:hidden py-5 flex-col gap-1.5"
        >
        ...
        {/*Itens NavBar */}
        <ul className="sm:flex hidden flex-1 items-center  justify-between  ">
          {liNav.map((data, id) => (
            <li key={id}>
              <Link href={`/${data.slug}`}>
